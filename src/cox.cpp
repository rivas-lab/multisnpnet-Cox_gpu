...
class Cox_GPU
{
    ...
    uint32_t K; // Number of responses
            ...
            int *rev_order): N(N), K(K), p(p)
    {
        ...
        Eigen::Map<Eigen::MatrixXd> Bmap(B0, p, K);
        ...
        cudaMemcpy(dev_param.B, &Bf(0,0), sizeof(numeric)*K*p, cudaMemcpyHostToDevice);
    ...
    void get_param(std::vector<cudaStream_t> &streams, cublasHandle_t handle)
    {
        ...
        cudaMemcpy(&host_B(0,0), dev_param.B, sizeof(numeric)*p*K, cudaMemcpyDeviceToHost);
    ...
    {
        update_parameters(dev_param, K, p, step_size, lambda_1, lambda_2);
    }
                       ...
                       )
{
    ...
    Cox_GPU prob(N, K, p, &X(0,0), &status(0,0), &B0(0,0), &pfac[0],
